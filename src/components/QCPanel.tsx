...
export function QCPanel({ qcResults, statsMap }: QCPanelProps) {
  ...
  return (
    <section className="qc-section">
      ...
      <p className="qc-source">
        ...
        — species-specific N50, contig count, genome size, GC%, Ns/100kbp.
      </p>
            ...
            {qcResults.map((r) => {
              ...
              return (
                <tr key={r.filename}>
                  ...
                  <td><StatCell value={stats?.contigCount} field="# contigs (>= 0 bp)" checks={r.checks} /></td>
                  <td><StatCell value={stats ? stats.totalLength / 1e6 : undefined} field="Total length (>= 0 bp)" checks={r.checks} fmt={(v) => v.toFixed(2)} /></td>
                  <td><StatCell value={stats?.gcPercent} field="GC (%)" checks={r.checks} fmt={(v) => v.toFixed(1)} /></td>
                  <td><StatCell value={stats?.nsPer100k} field="Ns per 100 kbp" checks={r.checks} fmt={(v) => v.toFixed(0)} /></td>
                </tr>
              )
            })}
          </tbody>
